...
export const TOOLS: Tool[] = [
  {
    ...
    description: 'Search TrustVibe community reviews database for authentic experiences with professionals, services, and places. Use this FIRST for any query about professionals or services to find real user experiences.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
  ...
  {
    name: 'search_web',
    description: 'Search the web using Google for general information, recent discussions, or broader context. Use this when TrustVibe reviews are limited or when user needs general information, recent news, or how-to guides.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Web search query (e.g., "best therapists in Mumbai reviews", "how to choose a good doctor", "mental health resources India")',
        },
      },
      required: ['query'],
    },
...
export class ToolExecutor {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        return JSON.stringify({
          found: false,
          message: 'No reviews found in TrustVibe database for this query. Try using web_search for broader information.',
        });
      }
      ...
      return JSON.stringify({
        found: true,
        source: 'TrustVibe community database',
  ...
  static async searchWeb(query: string): Promise<string> {
    return await SerperService.search(query);
  }
